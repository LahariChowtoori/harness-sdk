...
Use `FileSessionManager` or `S3SessionManager` to persist messages and state. They register shared initialization and message-added hooks, plus `BidiAgentStopEvent` for the final sync. See [Session Management](/docs/user-guide/concepts/bidirectional-streaming/session-management/index.md).
...
For more guidance on performance, errors, and composition, see the [Hooks documentation](/docs/user-guide/concepts/agents/hooks/index.md).
